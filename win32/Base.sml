...
in
  (*
    ...
    *)
    ...
    val C_GetLastError =
      _import "GetLastError" stdcall
      : unit -> C_ULong.t;

    fun GetLastError() : Word32.word =
      C_GetLastError()

    exception SystemError of Word32.word

    (* The string argument of the SysErr exception is supposed to match the result of OS.errMsg. *)
    fun raiseSysErr () = let val err = GetLastError() in raise SystemError err end
